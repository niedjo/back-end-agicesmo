...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // if(intval($reste_a_payer) !== 0){
        ...
        if ($x === 0 && intval($reste_a_payer) !== 0) { // s'il n'a pas deja de dette
            $sql1 = "INSERT INTO `screening_medicale_conta`(`code_personnel_scr`, `code_patient_scr`, `true_date`, `prix`, `quantitee`, `resultat`) 
                VALUES (:code_personnel_scr, :code_patient_scr, $DateActuele, :prix, :quantitee, :contenue);

                INSERT INTO `effectuer_op`(`code_personnel`, `id_caisse`, `motif`, `montant_aj`, `montant_re`) 
                VALUES (:code_personnel_scr, 1, :motif, :montant_aj, :montant_re);
            ...
            $requette1->bindParam(":code_personnel_scr", $idSaver);
            $requette1->bindParam(":code_patient_scr", $idPatient);
            // $requette1->bindParam(":nature_exam", $NatureExam);
