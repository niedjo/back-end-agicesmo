...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // if(intval($reste_a_payer) !== 0){
        ...
        if ($x === 0 && intval($reste_a_payer) !== 0) { // s'il n'a pas deja de dette
            $sql1 = "INSERT INTO `consultation`(`code_personnel_cons`, `code_patient_cons`, `true_date`, `type_de_consultation`, `quantitee`, `prix`, `resultat`, `status_don`) 
                VALUES (:code_personnel_cons, :code_patient_cons, $DateActuele, :nature_du_document, :quantitee, :prix, :resultat, 0);

                INSERT INTO `effectuer_op`(`code_personnel`, `id_caisse`, `motif`, `montant_aj`, `montant_re`) 
                VALUES (:code_personnel_cons, 1, :motif, :montant_aj, :montant_re);
            ...
            $requette1->bindParam(":code_patient_cons", $idPatient);
            $requette1->bindParam(":code_personnel_cons", $idSaver);
