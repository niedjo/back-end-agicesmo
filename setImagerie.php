...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // if(intval($reste_a_payer) !== 0){
        ...
        if ($x === 0 && intval($reste_a_payer) !== 0) { // s'il n'a pas deja de dette
            $sql1 = "INSERT INTO `imagerie`(`code_patient_imagerie`, `code_personnel_imagerie`, `true_date`, `nature_exam`, `quantitee`, `prix`) 
                VALUES (:code_patient_ima, :code_personnel_ima, $DateExam, :nature_exam, :quantitee, :prix);

                INSERT INTO `effectuer_op`(`code_personnel`, `id_caisse`, `motif`, `montant_aj`, `montant_re`) 
                VALUES (:code_personnel_ima, 1, :motif, :montant_aj, :montant_re);
            ...
            $requette1->bindParam(":code_patient_ima", $idPatient);
            $requette1->bindParam(":code_personnel_ima", $idSaver);
